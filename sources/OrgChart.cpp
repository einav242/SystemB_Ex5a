...
namespace ariel
{
    ...
    ostream &operator<<(ostream &out, const OrgChart &a)
    {
        ...
        while (!stack.empty())
        {
            ...
            if(tmp->parent==a.root && a.root->child!=tmp) //Whenever we finish exploring to a certain depth of a particular son of the root we go down a line
            {
                out<<"nullptr\n\n";
                for(unsigned int i=0;i<tmp->parent->info.size();i++)
                {
                    out<<" ";
                }
                out<<"-->";
