...
TEST_CASE("Good input")
{
    ...
    */
    ...
    /*the tree:
        Chief of police
        |                  |                   |
        Business Manager---Deputy chief--------Administrative support
                           |              |
    ...
    */
    ...
    OrgChart abc_organization;
   abc_organization.add_root("A")//third organization
        .add_sub("A", "B") //build the organiztion
        .add_sub("A", "C")
        .add_sub("A", "D")
        .add_sub("B", "E")
        .add_sub("B", "F").add_sub("B","G").add_sub("B","H").add_sub("E","I").add_sub("F","J").add_sub("F","K").add_sub("K","L");
    /*the tree:
        A
        |   |    |            
        B---C----D
    ...
    */
    ...
    i = 0;
    for (auto it =abc_organization.begin_level_order(); it != abc_organization.end_level_order(); ++it)//check level_order
    {
        CHECK(a[i] == (*it));
        i++;
    }
     a={"A","B","E","I","F","J","K","L","G","H","C","D"};
    i = 0;
    for (auto it =abc_organization.begin_preorder(); it != abc_organization.end_preorder(); ++it)//check preorder
    {
        CHECK(a[i] == (*it));
        i++;
    }
    a={"L","I","J","K","E","F","G","H","B","C","D","A"};
    i = 0;
    for (auto it =abc_organization.begin_reverse_order(); it != abc_organization.reverse_order(); ++it) //check rverse
    {
        CHECK(a[i] == (*it));
        i++;
    }
    ...
    i = 0;
    for (auto it =abc_organization.begin_level_order(); it != abc_organization.end_level_order(); ++it)//check size of all the item in the level order
    {
        CHECK(b[i] == it->size());
        i++;
    } 
...
TEST_CASE("Bad use of operators")
{
    ...
    OrgChart abc_organization;
    army_organization.add_root("A");
    //the first input does not exist in the tree
    CHECK_THROWS(abc_organization.add_sub("B","C"));
    CHECK_THROWS(abc_organization.add_sub("C","D"));
    CHECK_THROWS(abc_organization.add_sub("D","E"));
}